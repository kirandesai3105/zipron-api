...
function scoreProduct(source,candidate){
  ...
  const sourceIds=[source.gtin,source.mpn,source.sku].filter(Boolean).map(String).map(x=>x.toLowerCase());
  const candidateIds=[candidate.gtin,candidate.mpn,candidate.sku].filter(Boolean).map(String).map(x=>x.toLowerCase());
...
async function amazonToken(){
  ...
  const r=await fetch("https://api.amazon.co.uk/auth/o2/token",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({grant_type:"client_credentials",client_id:process.env.AMAZON_CLIENT_ID,client_secret:process.env.AMAZON_CLIENT_SECRET,scope:"creatorsapi::default"})});
...
async function searchAmazon(product){
  ...
  const r=await fetch("https://creatorsapi.amazon/catalog/v1/searchItems",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json","x-marketplace":marketplace},body:JSON.stringify(payload)});
...
async function searchFlipkart(product){
  ...
  const r=await fetch(url,{headers:{"Fk-Affiliate-Id":id,"Fk-Affiliate-Token":token}}); if(!r.ok)throw new Error(`Flipkart Search HTTP ${r.status}`);
...
async function savePriceHistory(product,offers,checkedAt){
  ...
  const r=await fetch(`${supabaseUrl.replace(/\/+$/i,"")}/rest/v1/price_history`,{method:"POST",headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(rows)});
...
async function compare(product){
  const results=[],errors=[]; const jobs=[["Amazon",searchAmazon(product)],["Flipkart",searchFlipkart(product)]]; const settled=await Promise.allSettled(jobs.map(x=>x[1]));
  ...
  if(product.price!=null)matched.push({store:product.store||"Current page",title:product.title,productId:product.productId||product.asin||"",price:Number(product.price),url:product.url||"",affiliateUrl:product.affiliateUrl||product.url||"",image:product.image||"",matchScore:1,note:"Current product page"});
  const unique=[],seen=new Set(); for(const x of matched){const key=`${x.store}|${x.productId||x.url||x.title}`;if(!seen.has(key)){seen.add(key);unique.push(x);}}
  unique.sort((a,b)=>a.price-b.price); const offers=unique.slice(0,10),checkedAt=new Date().toISOString(); let database={saved:false,reason:"Not attempted."};
...
app.get("/health",(_,res)=>res.json({ok:true,service:"zipron-price-api",version:"3.0.0"}));
